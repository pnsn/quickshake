...
function MongoArchive(ringBuff, interval, collection, logger) {
    this.ring = ringBuff;
    this.interval = interval;
    this.rtCol = collection;
    this.collectionRef = {}; //keep an key collection so we don't have to always lookup the colletions on mongo
...
MongoArchive.prototype.tail=function(){
...
MongoArchive.prototype.archiveScnl=function(key){
...
MongoArchive.prototype.createMongoCollection =function(key){
  var _this=this;
  _this.db.createCollection(key, { capped: true, size: _this.collectionSize},function(err, coll){
    if(err) throw err;
    _this.logger.info("creating mongodb collection wavforms." + key);
