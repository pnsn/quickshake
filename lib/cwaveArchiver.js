...
CwaveArchiver.prototype.addDocuments=function(cname){
  var collection = this.db.collection(cname);
  var buffs=[];
  var traces=this.ring.ring[cname]['traces'];
  var size =traces.length;
  var index = this.ring.ring[cname]['lastArchivedIndex'];
  var stopIndex = this.ring.ring[cname]['currentIndex'];
  while(index != stopIndex){
    index=(index+1)%size;
    buffs.push(traces[index]);
  }
  this.ring.ring[cname]['lastArchivedIndex']=index;
  if(buffs.length > 0){
