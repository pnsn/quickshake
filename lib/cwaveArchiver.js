...
function CwaveArchiver(ringBuff, interval, rtCol, logger) {
    this.ring = ringBuff;
    this.interval = interval;
    this.rtCol = rtCol;
    this.collectionRef = {}; //keep a key collection so we don't have to always lookup the colletions on mongo
    this.collectionSize =   ((3*86400* 1500)/256) * 256; //size in bytes ~3 days assumes sample size of ~1sec and package size of 1500 bytes
    this.logger = logger;
    this.db = null;
}
